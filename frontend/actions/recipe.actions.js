...
// Get recipe by Strapi ID (for cooking mode)
export async function getRecipeById(recipeId) {
  try {
    ...
    const normalizeRecipe = (entity) => {
      if (!entity) return null;
      // Strapi v4 sometimes returns { id, attributes: {...} }
      if (entity?.attributes) return { id: entity.id, ...entity.attributes };
      return entity;
    };

    // 1) Try canonical Strapi endpoint: /api/recipes/:id
    const idUrl = `${STRAPI_URL}/api/recipes/${encodeURIComponent(
      recipeId
    )}?populate=*`;
    ...
    if (response.ok) {
      ...
      const recipe = normalizeRecipe(json?.data);
      if (!recipe) throw new Error("Recipe not found");
      return { success: true, recipe, recipeId: recipe.id ?? recipeId };
    }

    // If the direct endpoint fails (often 404), try filter-based lookups.
    // This handles cases where the app passes Strapi `documentId` or similar identifiers.
    const firstErrorText = await response.text();
    console.warn(
      "⚠️ Failed to fetch recipe via /api/recipes/:id. Falling back to filters. Input id:",
      recipeId,
      "Error:",
      firstErrorText
    );

    const tryFilterLookup = async (field) => {
      const filterUrl = `${STRAPI_URL}/api/recipes?filters[${field}][$eq]=${encodeURIComponent(
        recipeId
      )}&populate=*`;
      ...
      return normalizeRecipe(entity);
    };

    const recipe =
      (await tryFilterLookup("id")) ||
      (await tryFilterLookup("documentId")) ||
      (await tryFilterLookup("document_id"));
